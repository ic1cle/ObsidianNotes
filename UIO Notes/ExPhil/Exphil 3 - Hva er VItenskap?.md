# Hva er vitenskap? Hva er vitenskapelig?

Se også: [[Oversikt|Oversikt]] · forrige: [[Exphil Seminar 2]] · neste: [[Exphil 4 - Kunnsapsresistens]]
*Hva betyr pseudovitenskap?*
	Falsk

*Pseudpvitenskap er en type falsk. (liksom) vitenskap*
selektiv og usunn skepsis:
	bevisbyrde for å avkrefte er høy
		og bevisbydre for å bekrefte er lav

Pseudovit. teorier
	- Astrologi
	- frenlogi
	- homøopati
	- kreasjonis,e (uintelligent design)
pseudovit prakss:
	- vage, overdrevne ikke testbare påstander
	- bevisfornektelse
	- lav standard for bevisinnsa,ling 
	- liten grad av åpenhet mot eksterne eksperter
	- konspiratorisk argumentasjon
	- bruk av villledende språk

## Men hva er da Vitenskap
*omtrentlig def: "Vitenskap er systematisk, metodisk og kritisk undersøkelse, studium eller forskning." - SNL*
*"Vitenskapen stiller krav om tilfredsstillende ecidens eller belegg for, eller begrunnelse av, de påstandene som fremstiller.*

Vitenskapen refere -> Natur- Sosial- Humaniora (åndsvitenskapene) - Vitenskap

Viktig å kunne skille mellom vitenskap og pseudovitenskap?
	Epistemiske grunner
		vilke toerier burde vi tro på?
	Etiske og politiske grunner
		Hvem er ekspertene som samfunnet kan stole på
		Hva gjør vi når folk nekter å tro på det ekspertene sier?
		Å kalle noe for "pseudo-vitenskap" kan være maktmisbruk

Kriterium -> demarkasjonskriterium
	vanskelig
	Ulike løsninger
		Konsensus
			*"Et Utsagn utgjør kunnskap dersom tilstrekkelig mange mennesker er tilstrekkelig overbeviste om det" (s65)*
			Menneskets historie o absurde ting alle trodde på
			Vitenskapelig konsensus????
				Forskere har vært enig om rare ting (geosentrisme, flogiston, osv.)
			En teoris vitenskapelig verdi har ingenting med hvor mange mennesker som tror på den
				*"Teeoriens verdi avhenger utelukkende av hvilken objektiv støtte disse hypotesene får av kjensgjerningene" (S66)"*
			Teorier bllir konfrontert med fakta, og en teori må underbygges av fakta
				debatt galileo scheiner
			Fakta vs Teori?
		Eksperimentell resonnering og induktiv metode
		probablisme
		falsifisering 

## Fakta og teorier
	Lakos siterer Hume, etterlyste "eksperimentell" argumentasjon
		Er det tilstrekkelig for vitenskap?
	På 1600-tallet -> Hekser
		Nøyaktige obsrv. og eksper.??
	Eksperimentell resonnering bruker induksjon
		etablerer generell påstand fra observerte enkelttilfeller
		Svane greia
		induksjonsåroblemet:
			Man kan aldri være sikker på den neste data man innhenter vil støtte konklusjonen
			Lakos sier at *"Alle vitenskapelige teorier er like **umulig** å bevise" (s67)*
		standarden for vitenskap bør ikke være for høy
			Descartes' kriterium *"hinsides all tvil"* er for strengt og "utopsik"
			Empirisk vitenskap kan ikke være like sikker som matematikk eller like nødvendig som deduktiv argumentasjon

## Probabilisme

Se også: [[Exphil 4 - Kunnsapsresistens#Infallibilisme og fallibilisme]] — fallibilisme senker begrunnelseskravet på samme måte som probabilisme senker sannsynlighetskravet
	Løse induksjonsproblemet ved å senke standard
		en teori krever høy sannsynlighet, ikke sikkerhet -> Vitenskap
	Poppers innvending
		*"Samtlige teoriers matematiske sannsynlighet, vitenskapelige som pseudovitenskapelige, er lik null"(S67)*
		*Insert docter house huh*
## Karl Popper
	Østeriske-britisk 1902-1994
	argument mot brobabilisme
		sannsynlighet = observasjoner / mulig evidens
		Mulig evidens er uendelig, observasjoner er bestemt og endelig, sannsynelighet tilnærmer seg altid null
	Hvordan skille mellem pseudo ok ikke???
		Ide: Istedenfor verifiserbarhet så kan vi benytte oss av falsifiserbarhet
			Hva betyr falsifiserbarhet - Hvordan løsning
				En Teori er falsifiserbar dersom det er mulig å sette opp en test eller eksperiment som kan vise at teorien er feil
					Ikke det samme som forfalsking
					sier noe om hvorvidt en teori kan motbevises
### Falsifiseringsprinsippet
					***En teori er vitenskapelig kun hvis den er falsifiserbar***
					Eksempel på falsifiserbarhetstest
						Einsteins relativitetseori
						Mulig test: Avbøying av lys på grunn av solen
							stor masse -> rom rundt sol bøye lys som går forbi
							lys beveger seg i rette linjer
							Hvis lyset ikke bøyer --> teori feil
								Hvordan bevise????
								solformørkelse?
									Tok bilder av stjerner som egentlig skulle vært gjemt
									einstein riktig
									publisert i 1920 førte til allmenn aksept av relativitetsteorien
									NB en teori som ikke består falsifisering kan være en utdatert eller feilaktig vitenskapelig teori
								***Marxistisk teori***
									Historisk materialisme
										Forsøk på å forklare menneskets historie ut ifra kampen om produksjonsmidlene mellom ulike sosiale klasser
										En forklaringsmodell for samfunnets og menneskets historie
										marx så på sin teori som vitenskap
									Marxistisk teori mer generelt
										samlebetegnelse for teorier, forklaringsmodeller og intellektuelle tradisjoner med opphav i kant
									***Er marxistisk teori en vitenskap eller pseudovitenskap?****
										For en Popper-tilhenger er marxismen vitenskapelig hvis marxister er villige til å spesifisere kjensgjerninger som ville til å få dem til åå oppgi marxisme skulle de foreligge. hvis de nekter -> pseudovit.
										Svaret avhenger marxisme falsifisering
					Innvendinger mot falsifiseringsprinsippet?
						Det stemmer ikke med hva vi ser i praksis!
							Vitenskapsforlk forkaster ikke teoriene sine basert på en enkelt motstridende observ.
							de er tykkhudede, kreative og motstandsdyktige
							*"De oppgir ikke en teori bare fordi den motsies av fakta" (s.68)*
						Hva kan forskere gjøre i møte ---
							- Beskrive dem som uregelmessig eller anomalier, som de ikke ennå kan forklare
							- Bruke det teoretiske rammeverket for å forklare at de ikke faktisk strider mot teorien
							- Endre deler av det teoretiske rammeverket får å tilpasse dem

## Thomas Kuhn
	amerikansk filosof 1922-1996
	påvirket [[#Karl Popper]]
	Vitenskap består av paradigmer som er det teoretiske rammeverket vitenskapfolk bruker for å forstå verden
		vitenskapelig fremskritt handler i stor grad om vitenskapelige revolusjoner hvor et paradignme blir forkaster til fordel
	Men vitenskapelige revolusjon sjelden, jobber innenfor paradigme (Normalvitenskap)
### Vitenskapelig paradigme
		Et paradigme er et rammeverk for å forstå verden - aksepterte teorier, metoder, lover,, antagelser og teknologier som forskere jobber innenfor og utenfor
		*Vitenskapelig utvikling*
			Pre-paradigmatisk fase
			Normalvitenskap med problemløsning
			Anomalier
			krise? - på tampen av revolusjon? 
			Vitenskapelig revolusjon *(feks einsteins stjerne  obsrv - newton)*
			Paradigmeskifte og nytt paradigme - Tilbake til normalvitenskap
Kuhn mot popper
	Falsifikasjon skjer hovedsakelig i forbindelse mer "revolusjonsfasen" som er relativt sjeldne anledninger
		Vi burde heller bruke normalvitenskaåern som utgangspunkt for demarkasjon
	krav om problemløsning
		Normal vitenskap -> problemløsning hbor teorie accept og problem løses
	Kuhn mot paradigmes usammenlignbarhet
		kuhn: paradigme før og etter revolusjon = usammenlignbare
			Ulike språk (mattematisk)?
			Kan ikke bruke nytt paradigme til å motbevise eller omvendt
	Lakatos sin innvending
		Kuuhn sitt syn gjør at endringer skjer som irrasjonell overtakelse
			Ingen gode argument for å endre syn????
		Rasjonelle overveielser står sentralt i vitenskapelig endring???
## Lakatos
	Se også: [[Exphil Seminar 2]] — seminarnotater om Lakatos
	Kritiserer popper og kuhn, men bygger videre på dem
		enig med popper i at vi trenger en forståelse av t-endring som er begrunnet
		Enig med kuhn i at vi mp ta hlyde for histoerie og faktisk praksis
### Forskningsprogrammer
		Komplekse rammeverk som rettleder vitenskapelg spørsmål, eksperimenter og tolkning av data
		Historiske enheter som eksisterer i en periode endrer seg over tid, løser problemer og takler anomalier
			- Newtons mekanikk
			- evolusjonsteorien
			- mikrobeteorien om sykdom
		Forskningsprogrammer vs. paradigmer
			Inkluderer flere beslektede teorier men er mindre enn paradigme
			viktig forskjell, hva som skjer når programmer er i endring
		Struktur til et forskningsprogramm
			Hard kjørne av uforanderlige teorier og antakelser
			Beskyttelsesbelte av foranderlige hjelpehypoteser og tilleggsantagelser
			Forbud mot å endre den harde kjærnen
			påbud som angir aksepterte måter å endre hjelpehyp. og tillegsantag. (Positiv heuristikk)
		Anomalier eller uregelmessigheter
			Alle teorier har uregelmessigheter
				betyr ikke at de er dårlige eller uvitenskapelig
			hvordan man takler anomalie

### Progressive og degenererende forskningsprogrammer
progressive:
	- Gjør nye overraskende prediksjoner
	- øker forklaringsevne
	- mot fremtid
	- Takler problematiske funn samtidig som de øker evnen til forklaring
Degenererende:
	- Ikke ny prediksjoner
	- øker ikke forklaringsevnen
	- fokusert på nåtid
	- ??????
Lakatos sitt demarkasjonskriterium
	Et forsknings program er vitenskapelig dersom det er progressivt og pseudovitenskap om degenererende
Endring i vitenskappen
	et degenererende vil dø langsomt etter flere forskere skifter til progressive

---
**Se også:** [[Exphil 4 - Kunnsapsresistens]] · [[Oversikt|Oversikt]]